...
class Orchestrator:
    ...
    def _real_recon(self, topic: str, workflow_name: str,
        ...
        prompt = (
            ...
            '      {\n'
            '        "id": "02_codegen",\n'
            '        "name": "Code generation with compile check",\n'
            '        "pattern": "pipeline",\n'
            '        "depends_on": ["01_research"],\n'
            '        "agents": [\n'
            '          {\n'
            '            "role": "codegen-writer",\n'
            '            "rationale": "Write code based on design",\n'
            ...
            "  - retry_role: different agent to retry (default: same agent)\\n"
            "  - fallback_role: agent to escalate to when retries exhausted\\n"
            "Include error_handler when the task requires compile/validation feedback loops.\\n"
